...
int SSAO(int argc, char* argv[]) {
    ...
    Shader shaderLightBox("DeferredLightBox.vert", "DeferredLightBox.frag");// 用于正向渲染光照盒子
...
#pragma endregion
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            //renderCube();
            //shaderGeometryPass.setInt("invertedNormals", 0);// 可能是因为在方块内部，法向量朝外而不是朝里，临时改动
